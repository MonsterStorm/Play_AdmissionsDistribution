...
public class PlatformController extends BaseController {
	...
	public static Result page(String page) {
		...
		if (PAGE_INDEX.equalsIgnoreCase(page)) {
			...
			// 教育机构注册
			return ok(views.html.module.platform.login
					.render(form(Login.class)));
	...
	public static Result pageAddEducation() {
		...
		if (user == null) {
			return ok(views.html.module.platform.addEducation.render(null, null));
	...
	public static Result pageAddAgent() {
		User user = LoginController.getSessionUser();
		if (user == null) {
			return ok(views.html.module.platform.addAgent.render(null, null));
		}
	...
	public static Result pageAddTeacher() {
		User user = LoginController.getSessionUser();
		if (user == null) {
			return ok(views.html.module.platform.addTeacher.render(null, null));
		}
	...
	public static Result pagePlatformStudentWords() {
		...
		Page<StudentWords> words = StudentWords.findPage(form()
	...
	public static Result pagePlatformTeacher() {
		...
		Page<Instructor> teachers = Instructor.findPage(form()
				.bindFromRequest(), page, null);
	...
	public static Result pagePlatformTeacherComp() {
		...
		Page<Instructor> instructor = Instructor.findPage(form()
				.bindFromRequest(), page, 12);
	...
	public static Result pagePlatformAdv() {
		...
		Page<Advertisment> advertisment = Advertisment.findPage(form()
				.bindFromRequest(), page, null);
	...
	public static Result pageEducation2() {
		...
		Page<EducationInstitution> education = EducationInstitution.findPage(
				form().bindFromRequest(), page, 5);

		FormHelper.resetFlash(form().bindFromRequest(), flash());

		return ok(views.html.module.platform.platformEducation2
				.render(education));
	}
	...
	public static Result pageUserEnroll() {
		...
		Long courseId = FormHelper
				.getLong(form().bindFromRequest(), "courseId");
		Course course = Course.find(courseId);
		User user = LoginController.getSessionUser();
		if (user == null) {
			// return badRequest(Constants.MSG_NOT_LOGIN);
			return ok(views.html.module.platform.platformUserEnroll.render(
					null, course, null, null, null));
		}
		...
		return ok(views.html.module.platform.platformUserEnroll.render(null,
	...
	public static Result pageContactUs() {
		...
		Page<Message> message = Message.findPage(form().bindFromRequest(),
				page, 5);
	...
	public static Result addOrUpdateStudentEnroll() {
		...
		if (student != null) {
			Long courseId = FormHelper.getLong(form().bindFromRequest(),
					"courseId");
			Course course = Course.find(courseId);
			if (course != null) {
				Enroll enroll = Enroll.findByStudentAndCourse(student, course);
		...
		if (form().bindFromRequest().get("agentId") != null) {
			Long agentId = FormHelper.getLong(form().bindFromRequest(),
					"agentId");
		...
		if (form != null && form.hasErrors() == false) {
			...
			if (student != null) {
				...
				if (form().bindFromRequest().get("courseId") != null) {
					Long courseId = FormHelper.getLong(
							form().bindFromRequest(), "courseId");
					Course course = Course.find(courseId);
				...
				Audit auditOfAgent = new Audit(student.user, Audit.STATUS_WAIT,
						AuditType.TYPE_AUDITTYPE_AGENT_ENROLL); // 代理人审核
				Audit auditOfEdu = new Audit(student.user, Audit.STATUS_WAIT,
						AuditType.TYPE_AUDITTYPE_EDU_ENROLL);// 教育机构的审核信息
				...
				return ok(views.html.module.platform.platformUserEnroll.render(
						enroll, enroll.course, enroll.fromAgent,
						enroll.student.user, enroll.student));
			}
	...
	public static Result pageEduInstList() {
		...
		Page<EducationInstitution> eduInst = EducationInstitution.findPage(
				form().bindFromRequest(), page, null);
	...
	public static Result pageEduInstIntroduction() {
		...
		// get page
		int page = FormHelper.getPage(form().bindFromRequest());
		Page<Course> course = Course.findPageByEducation(eduInst, form(), page,
				10);

		FormHelper.resetFlash(form().bindFromRequest(), flash());

		return ok(views.html.module.platform.eduInstIntroduction.render(
				eduInst, course));
	}
	...
	public static Result pageRegAgent() {
		...
		if (user == null) {
			long idt = (long) 2;
			Contract contractt = Contract.find(idt);
			return ok(views.html.module.platform.regAgent.render(contractt ));
		}
		if (user.agent != null && user.agent.audit != null
				&& user.agent.audit.status == Audit.STATUS_WAIT) {
	...
	public static Result pageRegTeacher() {
		...
		if (user == null) {
			long idt = (long) 3;
			Contract contractt = Contract.find(idt);
			return ok(views.html.module.platform.regTeacher.render(contractt));
		}
		if (user.instructor != null && user.instructor.audit != null
				&& user.instructor.audit.status == Audit.STATUS_WAIT) {
			return badRequest(Constants.MSG_AGENT_APPLYED_COURSE);
		}
	...
	public static Result addOrUpdateEducation() {
		String msg = Validator.check(PlatformController.class,
		...
		if (user == null) {
			...
			if (registerForm.hasErrors()) {
				return badRequest(registerForm.globalError().message());
			}else{
				user = User.addOrUpdate(form().bindFromRequest());
				if(user != null){
		...
		if (form != null && form.hasErrors() == false) {
			...
			if (edus == null) {
				EducationInstitution education2 = EducationInstitution
						.addOrUpdate(form.get());
				if (education2 != null) {
					List<EducationInstitution> list = new ArrayList<EducationInstitution>();
					list.add(education2);
					user.edus = list;
					user.update();
					return ok(views.html.module.platform.addEducation.render(
							education2, user));
				}
			} else {
				for (EducationInstitution edu : edus) {
					if (edu.name != null
							&& edu.name.equalsIgnoreCase(FormHelper.getString(
									form().bindFromRequest(), "name"))) {
		...
		if (form != null && form.hasErrors() == false) {
			...
			if (agent == null) {
				...
				if (agent2 != null) {
					user.agent = agent2;
					user.update();
					return ok(views.html.module.platform.addAgent.render(
							agent2, user));
				}
			} else {
	...
	public static Result addOrUpdateTeacher() {
		String msg = Validator.check(TeacherController.class,
				"addOrUpdateTeacher");
		if (msg != null) {
			return badRequest(msg);
		}
		...
		if (form != null && form.hasErrors() == false) {
			...
			if (instructor == null) {
				...
				if (instructor2 != null) {
					...
					user.update();
					return ok(views.html.module.platform.addTeacher.render(
							instructor2, user));
				}
			} else {
				instructor.info = FormHelper.getString(
						form().bindFromRequest(), "info");
				instructor.jobTitle = FormHelper.getString(form()
						.bindFromRequest(), "jobTitle");
				instructor.field = FormHelper.getString(form()
						.bindFromRequest(), "field");
